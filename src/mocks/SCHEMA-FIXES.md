...
| `user_profiles.profile_type` is a hard-coded 6-value enum mirrored in four places, and already wrong in three: `admin` and `staff` name no table, `guardian` names the same one as `parent` | **Agreed.** `DROP COLUMN profile_type` in GlobalDB and in the tenant replica. Add tenant tables `profile_types(code, name, capacity, is_builtin, is_active)` and `profile_profile_types(profile_id, profile_type_id, is_primary)` |
...
| `staff.designation` is one string, so it cannot say what somebody is when they are two things — the member of staff who is Bus Driver *and* Librarian has two `profile_profile_types` rows and one designation | Keep it, as the job title on the letterhead: free text, nullable, nobody's classification. The kinds a person is are `profile_profile_types`. Nothing may write a profile type's name into it — that put one fact in two columns and gave them room to disagree, which is `profile_type`'s mistake one table down |
