...
| `user_profiles.profile_type` is a hard-coded 6-value enum mirrored in four places, and already wrong in three: `admin` and `staff` name no table, `guardian` names the same one as `parent` | **Agreed.** `DROP COLUMN profile_type` in GlobalDB and in the tenant replica. Add tenant tables `profile_types(code, name, capacity, is_builtin, is_active)` and `profile_type_assignments(profile_id, profile_type_id, is_primary)` |
...
| One profile type per person breaks on the member of staff whose child attends the school | `profile_type_assignments` is plural, with `is_primary` for anywhere a UI needs one answer |
| Nothing makes a type assignment agree with the capacity row it names. `profile_types.capacity` says which table carries the record, but SQL cannot foreign-key a table *name*, so a profile can be assigned `teacher` with no `teachers` row — `typesOf` says teacher and `capacitiesOf` says nothing | A trigger on each capacity table's INSERT/DELETE that keeps the built-in assignment in step, so the row cannot be forgotten rather than being written twice by hand. The redundancy is the point — one query answers "what is this person" — and a constraint is what makes redundancy safe |
| `admin` and `staff` have no capacity table, so an accountant or receptionist has no record | Add `staff(profile_id, employee_id, joining_date, department, designation)`, `designation` NULL. Move those columns out of `teachers`, whose `profile_id` then references `staff(profile_id)` — a teacher is staff who teach |
| `staff.designation` is one string, so it cannot say what somebody is when they are two things — the member of staff who is Bus Driver *and* Librarian has two `profile_type_assignments` rows and one designation | Keep it, as the job title on the letterhead: free text, nullable, nobody's classification. The kinds a person is are `profile_type_assignments`. Nothing may write a profile type's name into it — that put one fact in two columns and gave them room to disagree, which is `profile_type`'s mistake one table down |
| `parents` is named for the commonest case, not for what it holds — 62 of the 1,129 rows across the two schools are an uncle. Which one somebody is is already in the link's `relationship` | **Agreed.** Rename to `guardians` / `student_guardians`, `parent_profile_id` → `guardian_profile_id`, capacity `parent` → `guardian`. Drop the `guardian` profile type: it named the same capacity as `parent` and said nothing more |
