/**
 * Kendriya Vidyalaya's notice board.
 *
 * Dates are relative to today — `relativeDisplay(-5)` for five days ago — so
 * the board is always current without a re-seed commit. The spread mimics a
 * real board: a couple pinned, several active, one scheduled, one draft, one
 * expired, one cancelled, because the board's own filters have to have
 * something to filter.
 *
 * Its own, because a notice is the most local thing a school produces. Shared,
 * both schools posted the same nine — the same choir rehearsal postponed for
 * the same auditorium renovation, in two cities.
 */
