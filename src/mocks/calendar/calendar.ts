...
/**
 * Build a YYYY-MM-DD string for a given day of the current calendar month.
 *
 * Used by the mock calendar below so events always land on real dates in the
 * month the app is being viewed — no hardcoded 2035 placeholders.
 */
function calDay(day: number): string {
  const now = new Date()
 ...
 *
 * Events are positioned via `calDay(N)` so they always land on day N of the
 * current calendar month — no hardcoded year or month.
 */
