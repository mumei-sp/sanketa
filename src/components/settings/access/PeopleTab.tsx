 ...
 * draft would risk overwriting another admin's edit on Save.
 *
...
export function PeopleTab({
  ...
  const changeRole = async (user: SchoolUser, roleId: string) => {
    ...
    if (await onPatch(user.id, { roleId })) {
      showSuccess(`${user.fullName} is now ${roles.find(r => r.id === roleId)?.name ?? roleId}`, {
        description: 'Takes effect at their next sign-in.',
      })
    }
  }

  const setClasses = (user: SchoolUser, next: string[]) => {
    void onPatch(user.id, { assignedClasses: next })
  }
                ...
                <div className="rounded-lg border p-2.5" style={{ borderColor: border.default }}>
                  <div className="mb-2 flex flex-wrap items-center gap-1.5">
                    ...
                    <button
                      ...
                      disabled={busy}
                      onClick={() =>
                        setClasses(user, assigned.length === classLabels.length ? [] : classLabels)
                      }
